...
def _category_totals(signals: List[dict], currency: str = "USD") -> dict:
    ...
    return {
        cat: {
            ...
            "open_impact_display": format_currency(v["open_impact"], currency),
            "resolved_impact": v["resolved_impact"],
            "resolved_impact_display": format_currency(v["resolved_impact"], currency),
        }
        for cat, v in totals.items()
    }


def _top_counterparties(records: List[dict], currency: str = "USD", top_n: int = 5) -> dict:
    ...
    customers = sorted(
        [{"name": k, **v, "invoiced_display": format_currency(v["invoiced"], currency), "outstanding_display": format_currency(v["outstanding"], currency)} for k, v in by_cust.items()],
        key=lambda x: x["invoiced"],
        reverse=True,
    )[:top_n]
    vendors = sorted(
        [{"name": k, **v, "spend_display": format_currency(v["spend"], currency)} for k, v in by_vendor.items()],
...
async def executive_report(current_user: dict = Depends(get_current_user)):
    ...
    cat_totals = _category_totals(signals, currency)
    top_cp = _top_counterparties(records, currency)
    ...
    top_actions = [
        {
            ...
            "impact_display": format_currency(s.get("impact_amount", 0), currency),
    ...
    return {
        ...
        "headline": {
            "revenue_recovered_display": format_currency(kpis["revenue_recovered"]["value"], currency),
            "revenue_recovered_amount": kpis["revenue_recovered"]["value"],
            "open_pipeline_display": format_currency(
                sum(v["open_impact"] for v in cat_totals.values()), currency
            ),
